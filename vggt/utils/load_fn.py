...
def load_and_preprocess_images_proportional(image_path_list, target_size=1024):
    """
    Load and preprocess images by resizing proportionally (long side to target size).
    Also returns the position information of original pixels after transformation.

    Args:
        image_path_list (list): List of paths to image files
        target_size (int, optional): Target size for the long side. Defaults to 1024.

    Returns:
        tuple: (
            torch.Tensor: Batched tensor of preprocessed images with shape (N, 3, H, W) 
                          (H/W is proportional to original, long side = target_size),
            torch.Tensor: Array of shape (N, 6) containing [x1, y1, x2, y2, width, height] for each image
                          (x1=0, y1=0 since no padding, x2=W, y2=H after resize)
        )
    ...
    for image_path in image_path_list:
        ...
        # Calculate scale factor (long side to target_size)
        max_dim = max(orig_width, orig_height)
        scale = target_size / max_dim

        # Calculate new dimensions after proportional resize
        new_width = int(orig_width * scale)
        new_height = int(orig_height * scale)

        # Resize the image proportionally (no padding)
        resized_img = img.resize((new_width, new_height), Image.Resampling.BICUBIC)

        # Coordinates: since no padding, original image fills the entire resized image
        x1, y1 = 0.0, 0.0
        x2, y2 = new_width, new_height

        # Store original image coordinates and original size
        original_coords.append(np.array([x1, y1, x2, y2, orig_width, orig_height]))

        # Convert to tensor
        img_tensor = to_tensor(resized_img)
        images.append(img_tensor)

    # Stack all images (note: shapes may be different if original aspect ratios differ)
    images = torch.stack(images)
    original_coords = torch.from_numpy(np.array(original_coords)).float()
