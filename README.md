...
但是我们发现在一些数据集上 VGGT 模型推理出的相机参数本身就有显著的误差，这一误差的存在会为重投影误差计算引入系统性误差，并且这一误差对当前相机参数有“偏好”。基于此，我们更换了一个新的轨迹过滤策略：
...
我们针对相机误差对重投影分布的影响进行过比较系统性的数学建模与仿真，并且对这一策略进行了实验验证，这些结果在一定程度上验证了这一改进的有效性，尤其在 VGGT 相机预测不准的场景下。此外，当相机精度较高，实验表明重投影误差分布的峰值将会非常接近0，我们的策略将退化为与原方法等效。
...
### 4.1 克隆仓库
```bash
# 由于项目中存在一个三百余帧视图的示例数据集，这个过程可能会偏慢
